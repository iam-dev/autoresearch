"""
Nanochat GPT model extracted from train.py.

This module contains all model classes and optimizer for the nanochat
architecture: GPTConfig, GPT (with RoPE, GQA, value embeddings, residual
lambdas, logit softcapping), and MuonAdamW optimizer.
"""
...
fa3 = None
if PLATFORM.supports_flash_attn:
    try:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        if has_flash_attn:
            ...
            # SDPA fallback: (B, T, nH, D) -> (B, nH, T, D) for SDPA
            ...
            y = y.transpose(1, 2).contiguous()  # back to (B, T, nH, D)
...
def build_model_config(
    ...
    """Build a GPTConfig from high-level hyperparameters.

    Args:
        vocab_size: Token vocabulary size (required).
        depth: Number of transformer layers.
        aspect_ratio: model_dim = depth * aspect_ratio.
        head_dim: Target head dimension; model_dim is rounded up to a multiple.
        window_pattern: Sliding window pattern string (e.g. "SSSL").
        sequence_len: Maximum sequence length.

    Returns:
        GPTConfig with derived n_embd, n_head, n_kv_head.
    """
