...
This is a fork of [karpathy/autoresearch](https://github.com/karpathy/autoresearch) that adds **[MnemeBrain](https://pypi.org/project/mnemebrain-lite/)** — a structured belief memory system for controlled ML experimentation. The original autoresearch idea: give an AI agent a small but real LLM training setup and let it experiment autonomously. It modifies the code, trains for 5 minutes, checks if the result improved, keeps or discards, and repeats. This fork layers a controlled experiment framework on top to scientifically measure whether structured belief memory (Belnap four-valued logic, contradiction detection, evidence-weighted confidence) helps an agent make better experiment decisions.

The training code is a simplified single-GPU implementation of [nanochat](https://github.com/karpathy/nanochat). More context on the original project in this [tweet](https://x.com/karpathy/status/2029701092347630069).

## How it works

The repo has two layers:

**Training layer** (from upstream autoresearch):
- **`prepare.py`** — fixed constants, one-time data prep (downloads training data, trains a BPE tokenizer), and runtime utilities (dataloader, evaluation). Not modified.
- **`train.py`** — training loop + hyperparameters. Contains the full GPT model, optimizer (Muon + AdamW), and training loop.
- **`program.md`** — baseline instructions for the agent.

**Experiment layer** (MnemeBrain additions):
- **`mnemebrain_hooks.py`** — 4 experimental conditions (A/B/C/D) that run between training runs, never during.
- **`results_analyzer.py`** — generates summaries, cross-condition comparisons, and plots from run results.
...
./scripts/launch.sh "Hi have a look at program.md and let's kick off a new experiment! let's do the setup first."
```
...
- **Self-contained.** No external dependencies beyond PyTorch and a few small packages (MnemeBrain packages are optional). No distributed training, no complex configs. One GPU, one file, one metric.
- **Between-run only.** MnemeBrain hooks execute before and after training, never during. The training loop is identical across all 4 conditions — this isolates the memory effect from optimizer effects.
